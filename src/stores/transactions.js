  ...
  actions: {
    ...
    buildTransactionFromResponse(transactionData) {
      const res = {
        ...
        t_payeeBankId: transactionData.t_payeeBankId,
        t_payeePayerAcctNo: transactionData.t_payeePayerAcctNo,
        t_originalCurrency: transactionData.t_originalCurrency,
        t_originalAmount: transactionData.t_originalAmount,
        t_exchangeRate: transactionData.t_exchangeRate,
    ...
    async updateTransactionCategories(transactionIds, categoryId) {
      const resultData = {};
      const userStore = UserStore();
      if (userStore.authenticated) {
        const config = userStore.getBearerAuthRequestHeader();
        ...
        const cId = categoryId === undefined ? null : parseInt(categoryId);
        if (isNaN(cId)) {
